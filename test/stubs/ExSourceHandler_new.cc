...
popcon::ExPedestalSource_new::ExPedestalSource_new(const edm::ParameterSet& pset) :
  m_name(pset.getUntrackedParameter<std::string>("name","ExPedestalSource_new")),
  p_(pset.getUntrackedParameter<unsigned int >("p")){
}

popcon::ExPedestalSource_new::~ExPedestalSource_new()
{
 
}

void popcon::ExPedestalSource_new::getNewObjects() {
   std::cerr << "------- " << m_name 
